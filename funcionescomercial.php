...
function fechasVencimientoProductos($codAlmacen, $cod_material){
	$estilosVenta=1;
	require("conexioncomercial.inc");
	$sql = "SELECT id.cod_material,GROUP_CONCAT(fecha_vencimiento)as fecha FROM ingreso_detalle_almacenes id
join ingreso_almacenes i on i.cod_ingreso_almacen=id.cod_ingreso_almacen
where i.ingreso_anulado=0 and id.cod_material='$cod_material' and i.cod_almacen='$codAlmacen'
...
function precioCostoProducto($codAlmacen, $cod_material){
	$estilosVenta=1;
	require("conexioncomercial.inc");
	$sql = "SELECT id.cod_material,id.costo_almacen FROM ingreso_detalle_almacenes id
join ingreso_almacenes i on i.cod_ingreso_almacen=id.cod_ingreso_almacen
where i.ingreso_anulado=0 and id.cod_material='$cod_material' and i.cod_almacen='$codAlmacen'
and id.cantidad_restante>0 order by i.fecha desc limit 1;";
	$resp=mysqli_query($enlaceCon,$sql);
	$costo='';
	while ($dat = mysqli_fetch_array($resp)) {
	  $costo=$dat['costo_almacen'];
	}
	return($costo);
}
?>